...
class AffineConstantFlow(nn.Module):
    ...
    def forward(self, x):
        s = self.s if self.s is not None else x.new_zeros(x.size()).to(x.device)
        t = self.t if self.t is not None else x.new_zeros(x.size()).to(x.device)
        z = x * torch.exp(s) + t
        log_det = torch.sum(s, dim=1)
        return z, log_det
    
    def backward(self, z):
        s = self.s if self.s is not None else z.new_zeros(z.size()).to(z.device)
        t = self.t if self.t is not None else z.new_zeros(z.size()).to(z.device)
        x = (z - t) * torch.exp(-s)
        log_det = torch.sum(-s, dim=1)
        return x, log_det
...
class AffineHalfFlow(nn.Module):
    ...
    def __init__(self, dim, parity, net_class=MLP, nh=24, scale=True, shift=True):
        ...
        self.parity = parity
        if dim % 2 == 0:
            dim1, dim2 = dim // 2, dim // 2
        else:
            if not self.parity:
                dim1, dim2 = (dim+1)//2, (dim-1)//2
            else:
                dim1, dim2 = (dim-1)//2, (dim+1)//2
        self.s_cond = lambda x: x.new_zeros(x.size(0), dim1)
        self.t_cond = lambda x: x.new_zeros(x.size(0), dim1)
        if scale:
            self.s_cond = net_class(dim1, dim2, nh)
        if shift:
            self.t_cond = net_class(dim1, dim2, nh)
...
class NormalizingFlowModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # prior_logprob = - zs[-1].pow(2).sum(-1) / 2
